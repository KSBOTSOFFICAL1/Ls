...
# ---------------------------------------------------------------------------
# JioTV M3U Playlist Core
# ---------------------------------------------------------------------------
# The playlist is used as a server-side channel index.  URLs are never sent
# back to Telegram users.  This layer is independent of the optional JioTV
# OTP/API integration, so /channels and M3U recording keep working even when
# the JioTV API session is unavailable.

JIOTV_PLAYLIST_URL = _environ.get(
    "JIOTV_PLAYLIST_URL",
    "https://raw.githubusercontent.com/Sflex0719/m3u/refs/heads/main/Zio.m3u",
)
JIOTV_CACHE_FILE = join(DATA_DIRECTORY, "jiotv_m3u_cache.json")


class AdvancedJioTVPlaylistEngine:
    def __init__(self, data_dir: str):
        self.data_dir = data_dir
        self.cache_file = join(data_dir, "jiotv_m3u_cache.json")
        self.channels_map: dict[str, dict] = {}
        self.last_refresh_ts = 0
        self.source_url = JIOTV_PLAYLIST_URL
        self.load_cache()
    ...
    def load_cache(self) -> None:
        try:
            ...
            with open(self.cache_file, "r", encoding="utf-8") as fh:
                data = json.load(fh)
            self.channels_map = data.get("channels", {}) or {}
            self.last_refresh_ts = int(data.get("updated_at", 0) or 0)
            self.source_url = data.get("source_url") or JIOTV_PLAYLIST_URL
            LOG.info("Loaded %d JioTV M3U channels from cache", len(self.channels_map))
        except Exception as exc:
            LOG.warning("Could not load JioTV M3U cache: %s", exc)
            self.channels_map = {}
    ...
    def save_cache(self) -> None:
        try:
            ...
            with open(tmp, "w", encoding="utf-8") as fh:
                json.dump({
                    "updated_at": self.last_refresh_ts,
                    "source_url": self.source_url,
                    "channels": self.channels_map,
                }, fh, indent=2, ensure_ascii=False)
            os.replace(tmp, self.cache_file)
        except Exception as exc:
            LOG.warning("Could not save JioTV M3U cache: %s", exc)
    ...
    def _parse_extinf(self, line: str) -> dict:
        # Handles quoted attributes plus the display-name after the final comma.
        attrs = {}
        for key, value in re.findall(r'([\w-]+)="([^"]*)"', line):
            attrs[key.lower()] = self._clean_attr(value)

        display = ""
        if "," in line:
            display = line.rsplit(",", 1)[1].strip()
        display = attrs.get("tvg-name") or display or "Unknown Channel"

        group = attrs.get("group-title") or attrs.get("group") or "General"
        channel_id = (
            attrs.get("tvg-id") or attrs.get("channel-id") or
            hashlib.md5(display.lower().encode("utf-8")).hexdigest()[:10]
        )
        return {
    ...
    def _parse_playlist(self, content: str) -> dict[str, dict]:
        if "#EXTM3U" not in content[:4096].upper():
            raise ValueError("Downloaded file is not a valid M3U playlist")
        ...
        for raw in content.splitlines():
            ...
            if line.startswith("#EXTINF:"):
                ...
                current["user_agent"] = pending_ua
                current["referer"] = pending_ref
                current["cookie"] = pending_cookie
                current["origin"] = pending_origin
                pending_ua = pending_ref = pending_cookie = pending_origin = ""
                continue

            # VLC/Kodi playlist header directives may appear immediately before
            # the URL. Keep the common HTTP header variants for FFmpeg.
            low = line.lower()
            if low.startswith("#extvlcopt:http-user-agent="):
                pending_ua = line.split("=", 1)[1].strip()
                continue
            if low.startswith("#extvlcopt:http-referrer=") or low.startswith("#extvlcopt:http-referer="):
                pending_ref = line.split("=", 1)[1].strip()
                continue
            if low.startswith("#extvlcopt:http-cookie="):
                pending_cookie = line.split("=", 1)[1].strip()
                continue
            if low.startswith("#extvlcopt:http-origin="):
                pending_origin = line.split("=", 1)[1].strip()
                continue
            ...
            if current and re.match(r"^https?://", line, re.I):
                current["url"] = line
                # Stable key; duplicate names are retained with a suffix.
                base = current["channelName"].strip().lower() or "unknown"
                key = base
                n = 2
                while key in new_map:
                    key = f"{base} #{n}"
                    n += 1
                new_map[key] = dict(current)
                current = None

        return new_map

    def sync_playlist_sync(self) -> bool:
        try:
            req = urllib.request.Request(
                self.source_url,
                headers={
                    "User-Agent": DEFAULT_USER_AGENT if "DEFAULT_USER_AGENT" in globals()
                    else "Mozilla/5.0",
            ...
            new_map = self._parse_playlist(content)
            if not new_map:
                raise ValueError("Playlist contains no channel URL entries")
            self.channels_map = new_map
            self.last_refresh_ts = int(time.time())
            self.save_cache()
            LOG.info("JioTV M3U refresh successful: %d channels", len(new_map))
            return True
        except Exception as exc:
            LOG.exception("JioTV M3U refresh failed: %s", exc)
            return False

    async def refresh_playlist(self) -> bool:
        return await asyncio.to_thread(self.sync_playlist_sync)

    def is_logged_in(self) -> bool:
        # Compatibility with the old JioTV API handlers.
        return bool(self.channels_map)
    ...
    def search_channel(self, query: str) -> list:
        ...
        return [v for k, v in self.channels_map.items() if q in k or q in v.get("channelName", "").lower()]
    ...
    def get_stream_url(self, channel_id: str) -> dict:
        ...
        return {"success": False, "message": "Channel stream URL not found in M3U playlist."}

    def get_channel_by_name(self, name: str) -> Optional[dict]:
        matches = self.search_channel(name)
        return matches[0] if matches else None

    def send_otp(self, phone: str) -> dict:
        return {"success": False, "message": "M3U mode does not require JioTV OTP login."}

    def verify_otp(self, otp: str) -> dict:
        return {"success": False, "message": "M3U mode does not require JioTV OTP verification."}

    def logout(self) -> None:
        # Keep the cached playlist available; there is no account session in M3U mode.
        return None

    def get_catchup_url(self, channel_id: str, begin_ts: int, end_ts: int) -> dict:
        # A live M3U playlist gives a current stream URL; it does not provide
        # historical catch-up segments by itself.
        return {
            "success": False,
            "message": "M3U playlist mode provides live streams only; historical catch-up requires a separate catch-up API.",
        }


jiotv = AdvancedJioTVPlaylistEngine(DATA_DIRECTORY)
...
async def rec_cmd(client: Client, message: Message):
    ...
    # JioTV M3U shortcut:
    #   /rec -Jiotv Pogo 01:00
    #   /rec -Jiotv Pogo 01:00 Pogo_File
    m3u_match = re.match(
        r"^/rec(?:@\S+)?\s+-Jiotv\s+(.+?)\s+(\d{1,2}:\d{2}(?::\d{2})?|\d+[smh])(?:\s+(.+))?$",
        msg_text.strip(), re.IGNORECASE,
    )
    if m3u_match:
        ...
        channel_query = m3u_match.group(1).strip()
        duration_token = m3u_match.group(2).strip()
        filename = (m3u_match.group(3) or channel_query).strip()
        duration_sec = _parse_duration_token(duration_token)
        if duration_sec <= 0:
            return await message.reply_text("❌ Duration galat. Example: `/rec -Jiotv Pogo 01:00`")

        # Keep the same basic recording limits used by /jiorec.
        if user_id not in OWNER_IDS:
            max_sec = 2 * 3600 if is_premium(user_id) else 3600
            if duration_sec > max_sec:
                return await message.reply_text(
                    f"⏱ Maximum recording duration: `{_seconds_to_hms(max_sec)}`.\n"
                    f"Requested: `{_seconds_to_hms(duration_sec)}`"
                )

        if not jiotv.is_logged_in():
            await jiotv.refresh_playlist()
        matched = jiotv.get_channel_by_name(channel_query)
        if not matched:
            return await message.reply_text(
                f"❌ JioTV M3U channel **{channel_query}** nahi mila.\n\n"
                f"Use `/channels -Jiotv {channel_query}` ya `/RefreshJiotv`."
            )

        result = jiotv.get_stream_url(matched.get("channel_id", ""))
        if not result.get("success") or not result.get("url"):
            return await message.reply_text("❌ M3U stream URL available nahi hai.")

        ch_label = matched.get("channelName") or channel_query
        safe_name = re.sub(r"[^\w\s.-]", "", filename).strip() or ch_label
        flags = {
        ...
        setup = _build_setup(
            user_id, message, result["url"], _seconds_to_hms(duration_sec), safe_name, flags
        )
        setup.update({
            ...
            "jiotv": True,
            "jiotv_m3u": True,
            "jiotv_channel": ch_label,
            "watermark_on": False,
        })
        await message.reply_text(
            f"📡 **JioTV M3U**\n"
            f"📺 Channel: `{ch_label}`\n"
            f"⏱ Duration: `{_seconds_to_hms(duration_sec)}`\n"
            f"▶️ Recording start ho rahi hai…"
        )
...
_BOT_COMMANDS = [
    ...
    BotCommand("RefreshJiotv",    "Refresh JioTV M3U playlist"),
]
...
# /RefreshJiotv — refresh the remote M3U playlist
@app.on_message(filters.command(["RefreshJiotv", "refreshjiotv", "RefreshJioTV"]) & AUTH)
async def refresh_jiotv_playlist_cmd(client: Client, message: Message):
    status = await message.reply_text("🔄 **JioTV M3U playlist refresh ho rahi hai…**")
    ok = await jiotv.refresh_playlist()
    if not ok:
        return await status.edit_text(
            "❌ **Playlist refresh failed.**\n"
            "Source URL check karo ya thodi der baad retry karo."
        )
    await status.edit_text(
        f"✅ **JioTV M3U refreshed!**\n\n"
        f"📺 Channels: `{len(jiotv.get_channels())}`\n"
        f"🕐 Updated: `{datetime.now(tz).strftime('%d-%m-%Y %I:%M:%S %p')}`"
    )
...
async def jiotv_channels_cmd(client: Client, message: Message):
    uid = message.from_user.id
    status = await message.reply_text("📡 JioTV M3U channels load kar raha hoon…")
    if not jiotv.is_logged_in():
        await jiotv.refresh_playlist()
    channels = await asyncio.get_event_loop().run_in_executor(None, jiotv.get_channels)
    if not channels:
        return await status.edit_text(
            "❌ JioTV M3U playlist load nahi hui.\nOwner `/RefreshJiotv` chalao aur dobara try karo."
        )

    # `/channels -Jiotv <search>` is a direct M3U search mode.
    raw_args = message.command[1:] if len(message.command) > 1 else []
    if raw_args and raw_args[0].lower() == "-jiotv":
        query = " ".join(raw_args[1:]).strip()
        matches = await asyncio.get_event_loop().run_in_executor(
            None, jiotv.search_channel, query
        )
        if not matches:
            return await status.edit_text(
                f"❌ JioTV M3U mein **{query or 'all channels'}** nahi mila."
            )
        lines = [f"📺 **JioTV M3U — {len(matches)} result(s)**\n"]
        for ch in matches[:30]:
            lines.append(
                f"• **{ch.get('channelName', '?')}** | `{ch.get('category', 'General')}`"
            )
        if len(matches) > 30:
...
async def jiotv_rec_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if not jiotv.is_logged_in():
            return await message.reply_text(
                "❌ **JioTV M3U playlist available nahi hai.**\n\n"
                "Owner `/RefreshJiotv` chalao aur phir retry karo."
            )
        args = message.command
        if len(args) < 3:
            return await message.reply_text(
...
async def jiotv_status_cmd(client: Client, message: Message):
    ...
    if not jiotv.is_logged_in():
        return await message.reply_text(
            "❌ JioTV M3U playlist unavailable hai. Owner `/RefreshJiotv` chalao."
        )
    await message.reply_text(
        "✅ **JioTV M3U Active**\n\n"
        f"📺 Channels: `{len(jiotv.get_channels())}`\n"
        "\nCommands:\n"
        "• `/channels -Jiotv [search]` — M3U channel search\n"
        "• `/jiochannels` — category browser\n"
        "• `/jiorec <channel> <duration>` — live record\n"
        "• `/rec -Jiotv <channel> <duration>` — direct record\n"
        "• `/RefreshJiotv` — refresh playlist"
    )
