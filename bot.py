...
# Permanent Local Bot API mode: the bot must never silently fall back to
# Telegram's cloud API (which has the 20 MB download limit).
TELEGRAM_LOCAL_API_ENABLED = True
# Local Bot API can be pointed at an external service, or started automatically
# inside this container when API_ID/API_HASH and the telegram-bot-api binary exist.
TELEGRAM_LOCAL_API_DIR = os.environ.get("TELEGRAM_LOCAL_API_DIR", "/tmp/telegram-bot-api")
TELEGRAM_LOCAL_API_TEMP_DIR = os.environ.get("TELEGRAM_LOCAL_API_TEMP_DIR", "/tmp/telegram-bot-api-tmp")
...
def _start_local_telegram_api():
    """Start and require the bundled Telegram Local Bot API.

    This is intentionally fail-closed: if Local Bot API cannot start, the bot
    exits instead of falling back to api.telegram.org and hitting the 20 MB
    cloud download limit.
    """
    global TELEGRAM_LOCAL_API_URL, TELEGRAM_LOCAL_API_ENABLED
    global TELEGRAM_BOT_DOWNLOAD_LIMIT, TELEGRAM_BOT_UPLOAD_LIMIT
    global _LOCAL_TELEGRAM_API_PROCESS

    if not API_ID or not API_HASH:
        raise RuntimeError(
            "Permanent Local Bot API mode requires API_ID and API_HASH. "
            "Cloud Bot API fallback is disabled."
        )

    # If an explicit external local endpoint is supplied, use it. Never accept
    # the cloud endpoint as a local endpoint.
    if TELEGRAM_LOCAL_API_URL:
        if "api.telegram.org" in TELEGRAM_LOCAL_API_URL.lower():
            raise RuntimeError(
                "TELEGRAM_LOCAL_API_URL points to api.telegram.org. "
                "Permanent Local Bot API mode requires a local/self-hosted endpoint."
            )
        TELEGRAM_LOCAL_API_ENABLED = True
        TELEGRAM_BOT_DOWNLOAD_LIMIT = 2 * 1024 * 1024 * 1024
        TELEGRAM_BOT_UPLOAD_LIMIT = 2 * 1024 * 1024 * 1024
        logger.info("Permanent external Telegram Local Bot API enabled: %s", TELEGRAM_LOCAL_API_URL)
        return True
    ...
    try:
        ...
        raise RuntimeError(f"Unable to start Telegram Local Bot API: {exc}") from exc
    ...
    while time.monotonic() < deadline:
        if _LOCAL_TELEGRAM_API_PROCESS.poll() is not None:
            ...
            _LOCAL_TELEGRAM_API_PROCESS = None
            raise RuntimeError(f"Telegram Local Bot API exited during startup (code {code}).")
        try:
            with socket.create_connection(("127.0.0.1", TELEGRAM_LOCAL_API_PORT), timeout=0.5):
                break
        except OSError:
            time.sleep(0.25)
    else:
        ...
        _LOCAL_TELEGRAM_API_PROCESS = None
        raise RuntimeError("Telegram Local Bot API did not open its port within 20 seconds.")

    TELEGRAM_LOCAL_API_URL = f"http://127.0.0.1:{TELEGRAM_LOCAL_API_PORT}"
    TELEGRAM_LOCAL_API_ENABLED = True
    TELEGRAM_BOT_DOWNLOAD_LIMIT = 2 * 1024 * 1024 * 1024
    TELEGRAM_BOT_UPLOAD_LIMIT = 2 * 1024 * 1024 * 1024
    logger.info("PERMANENT Local Telegram Bot API enabled; 2 GB upload/download safety limit.")
    return True
...
def main():
    ...
    # Local Bot API is mandatory; never fall back to the cloud API.
    ...
    if TELEGRAM_LOCAL_API_ENABLED:
        ...
        raise RuntimeError("Permanent Local Bot API mode failed to configure the local endpoints.")
