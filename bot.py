...
# Files at or below this size keep using Telegram's normal cloud Bot API.
TELEGRAM_CLOUD_FILE_LIMIT = 20 * 1024 * 1024
# Telegram Local Bot API supports files up to 2 GB. Keep one hard ceiling for
# both downloads and uploads so no code path can start an unbounded transfer.
TELEGRAM_MAX_FILE_SIZE = 2 * 1024 * 1024 * 1024
TELEGRAM_BOT_DOWNLOAD_LIMIT = TELEGRAM_MAX_FILE_SIZE
TELEGRAM_BOT_UPLOAD_LIMIT = TELEGRAM_MAX_FILE_SIZE

# The short names are the public configuration contract. The older names are
# still accepted so existing deployments keep working during the migration.
LOCAL_BOT_API = (
    os.environ.get("LOCAL_BOT_API")
    or os.environ.get("TELEGRAM_LOCAL_API_URL")
    or "http://127.0.0.1:8081"
).rstrip("/")
LOCAL_BOT_API_ENABLED = _env_bool("LOCAL_BOT_API_ENABLED", True)
if "LOCAL_BOT_API_ENABLED" not in os.environ:
    LOCAL_BOT_API_ENABLED = _env_bool("TELEGRAM_LOCAL_API_ENABLED", LOCAL_BOT_API_ENABLED)
LOCAL_BOT_API_REQUIRED = _env_bool("LOCAL_BOT_API_REQUIRED", True)
if "LOCAL_BOT_API_REQUIRED" not in os.environ:
    LOCAL_BOT_API_REQUIRED = _env_bool("TELEGRAM_LOCAL_API_REQUIRED", LOCAL_BOT_API_REQUIRED)

# Legacy aliases are kept for the existing setup helpers and documentation.
TELEGRAM_LOCAL_API_URL = LOCAL_BOT_API
TELEGRAM_LOCAL_API_AUTOSTART = _env_bool(
    "TELEGRAM_LOCAL_API_AUTOSTART", True
)
TELEGRAM_LOCAL_API_DIR = os.environ.get("TELEGRAM_LOCAL_API_DIR", "/tmp/telegram-bot-api")
TELEGRAM_LOCAL_API_TEMP_DIR = os.environ.get(
    "TELEGRAM_LOCAL_API_TEMP_DIR", "/tmp/telegram-bot-api-tmp"
)
TELEGRAM_LOCAL_API_PORT = int(os.environ.get("TELEGRAM_LOCAL_API_PORT", "8081"))
_LOCAL_TELEGRAM_API_PROCESS = None
_LOCAL_TELEGRAM_BOT = None
_LOCAL_TELEGRAM_API_AVAILABLE = False


class TelegramFileTooLargeError(RuntimeError):
    """Raised before a transfer starts when the 2 GB safety limit is exceeded."""


class LocalBotApiRequiredError(RuntimeError):
    """Raised when a file requires Local Bot API but it cannot be used."""


def _telegram_file_limit_error(file_size: int | None, label: str = "File") -> str | None:
    """Return a user-facing error for a known Telegram file size, if needed."""
    size = int(file_size or 0)
    if size <= 0:
        return None
    if size > TELEGRAM_MAX_FILE_SIZE:
        return (
            f"❌ {label} is larger than the supported 2 GB limit.\n\n"
            "The download or upload was not started."
        )
    if size > TELEGRAM_CLOUD_FILE_LIMIT and not LOCAL_BOT_API_ENABLED:
        if LOCAL_BOT_API_REQUIRED:
            return (
                "❌ Local Bot API is disabled.\n\n"
                f"Files larger than 20 MB require Local Bot API at {LOCAL_BOT_API}."
            )
        return (
            f"❌ {label} is larger than the normal Telegram Bot API 20 MB limit.\n\n"
            "Enable Local Bot API to process larger files."
        )
    return None


def _local_bot_api_error(exc: Exception) -> LocalBotApiRequiredError:
    return LocalBotApiRequiredError(
        "❌ Local Bot API is unavailable.\n\n"
        f"Please start the Telegram Local Bot API server on:\n{LOCAL_BOT_API}"
    )


def _local_telegram_bot() -> Bot:
    """Build the separate PTB client used only for large-file transfers."""
    global _LOCAL_TELEGRAM_BOT
    if _LOCAL_TELEGRAM_BOT is None:
        if not BOT_TOKEN:
            raise LocalBotApiRequiredError("TELEGRAM_BOT_TOKEN is not set.")
        _LOCAL_TELEGRAM_BOT = Bot(
            token=BOT_TOKEN,
            base_url=f"{LOCAL_BOT_API}/bot",
            base_file_url=f"{LOCAL_BOT_API}/file/bot",
        )
    return _LOCAL_TELEGRAM_BOT


def _transfer_bot(context, file_size: int | None, label: str = "File"):
    """Choose cloud Bot API for small files and Local Bot API for large files."""
    error = _telegram_file_limit_error(file_size, label)
    if error:
        if int(file_size or 0) > TELEGRAM_MAX_FILE_SIZE:
            raise TelegramFileTooLargeError(error)
        raise LocalBotApiRequiredError(error)
    if int(file_size or 0) <= TELEGRAM_CLOUD_FILE_LIMIT:
        return context.bot
    if not LOCAL_BOT_API_ENABLED:
        return context.bot
    try:
        return _local_telegram_bot()
    except LocalBotApiRequiredError:
        raise
    except Exception as exc:
        raise _local_bot_api_error(exc) from exc


async def _download_telegram_file(
    context, file_id: str, destination: str, file_size: int | None = None,
    label: str = "File",
):
    """Download to disk without buffering the Telegram file in memory."""
    transfer_bot = _transfer_bot(context, file_size, label)
    try:
        telegram_file = await transfer_bot.get_file(
            file_id,
            read_timeout=600,
            write_timeout=600,
            connect_timeout=60,
    ...
    except (TelegramFileTooLargeError, LocalBotApiRequiredError):
        raise
    except Exception as exc:
        if int(file_size or 0) > TELEGRAM_CLOUD_FILE_LIMIT and LOCAL_BOT_API_ENABLED:
            raise _local_bot_api_error(exc) from exc
        raise


def _upload_bot(context, file_size: int, label: str = "File"):
    """Choose the upload endpoint from the output file size."""
    return _transfer_bot(context, file_size, label)


def _start_local_telegram_api():
    """Optionally start the bundled Local Bot API without hiding outages.

    The bot still starts when the local service is unavailable so small files
    can use the normal Bot API and large-file requests can return the useful
    Local Bot API unavailable message from the transfer pipeline.
    """
    global TELEGRAM_LOCAL_API_URL, LOCAL_BOT_API, _LOCAL_TELEGRAM_API_AVAILABLE
    global _LOCAL_TELEGRAM_API_PROCESS

    if not LOCAL_BOT_API_ENABLED:
        logger.info("Telegram Local Bot API is disabled by LOCAL_BOT_API_ENABLED.")
        return False

    if "api.telegram.org" in LOCAL_BOT_API.lower():
        message = (
            "LOCAL_BOT_API must point to a Telegram Local Bot API server, "
            "not api.telegram.org."
        )
        if LOCAL_BOT_API_REQUIRED:
    ...
    # An already-running service, including a remote service, needs no child.
    try:
        parsed = urlsplit(LOCAL_BOT_API)
        if parsed.hostname not in {"127.0.0.1", "localhost"}:
            _LOCAL_TELEGRAM_API_AVAILABLE = True
            logger.info("Telegram Local Bot API configured at %s.", LOCAL_BOT_API)
            return True
        port = parsed.port or TELEGRAM_LOCAL_API_PORT
        with socket.create_connection((parsed.hostname, port), timeout=0.5):
            _LOCAL_TELEGRAM_API_AVAILABLE = True
            logger.info("Telegram Local Bot API is available at %s.", LOCAL_BOT_API)
            return True
    except (OSError, ValueError):
        pass

    if not TELEGRAM_LOCAL_API_AUTOSTART:
        logger.warning("Telegram Local Bot API is not reachable at %s.", LOCAL_BOT_API)
        return False
    if not API_ID or not API_HASH:
        logger.warning(
            "Telegram Local Bot API is not reachable at %s. "
            "Set API_ID/API_HASH to allow automatic startup, or start the service separately.",
            LOCAL_BOT_API,
        )
        return False

    try:
        os.makedirs(TELEGRAM_LOCAL_API_DIR, exist_ok=True)
        os.makedirs(TELEGRAM_LOCAL_API_TEMP_DIR, exist_ok=True)
        ...
        _LOCAL_TELEGRAM_API_PROCESS = subprocess.Popen(
            [
                ...
                f"--dir={TELEGRAM_LOCAL_API_DIR}",
                f"--temp-dir={TELEGRAM_LOCAL_API_TEMP_DIR}",
                "--verbosity=1",
            ],
        ...
        logger.warning("Unable to start Telegram Local Bot API: %s", exc)
        return False

    deadline = time.monotonic() + 20
    while time.monotonic() < deadline:
        if _LOCAL_TELEGRAM_API_PROCESS.poll() is not None:
            code = _LOCAL_TELEGRAM_API_PROCESS.returncode
            _LOCAL_TELEGRAM_API_PROCESS = None
            logger.warning(
                "Telegram Local Bot API exited during startup (code %s).", code
            )
            return False
        try:
            parsed = urlsplit(LOCAL_BOT_API)
        ...
        _LOCAL_TELEGRAM_API_PROCESS = None
        logger.warning(
            "Telegram Local Bot API did not open its port within 20 seconds at %s.",
            LOCAL_BOT_API,
        )
        return False

    LOCAL_BOT_API = f"http://127.0.0.1:{TELEGRAM_LOCAL_API_PORT}"
    TELEGRAM_LOCAL_API_URL = LOCAL_BOT_API
    _LOCAL_TELEGRAM_API_AVAILABLE = True
    logger.info("Telegram Local Bot API started at %s; 2 GB safety limit enabled.", LOCAL_BOT_API)
    return True
...
async def qualitymax_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    size_error = _telegram_file_limit_error(source["file_size"], "Source video")
    if size_error:
        await update.message.reply_text(
            size_error,
        )
...
async def merge_video_audio_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not source:
        ...
        return
    size_error = _telegram_file_limit_error(source["file_size"], "The video")
    if size_error:
        await update.message.reply_text(
            size_error,
        )
...
async def merge_audio_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not audio:
        ...
        return
    size_error = _telegram_file_limit_error(audio["file_size"], "The audio")
    if size_error:
        await update.message.reply_text(
            size_error,
        )
...
async def _run_merge_job(update, context, pending, audio, status_message, task_id):
    ...
    try:
        await _download_telegram_file(
            context,
            pending["video_file_id"],
            input_video,
            pending.get("video_size", 0),
            "The video",
        )
        await _download_telegram_file(
            context,
        ...
        try:
            upload_bot = _upload_bot(
                context, os.path.getsize(output_path), "Merged file"
            )
    ...
    try:
        ...
        if not ok:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) >= 1024:
                ...
                try:
                    upload_bot = _upload_bot(
                        context, os.path.getsize(output_path), "Partial recording"
                    )
        ...
        try:
            upload_bot = _upload_bot(
                context, os.path.getsize(output_path), "Converted video"
            )
...
async def _stream_upload_one(context, info: dict, status_message, output_path: str):
    ...
    with open(output_path, "rb") as raw:
        upload_bot = _upload_bot(
            context, os.path.getsize(output_path), "Extracted file"
        )
...
async def stream_extractor_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not source:
        ...
        return
    size_error = _telegram_file_limit_error(source.get("file_size"), "The video")
    if size_error:
        await update.message.reply_text(size_error)
        return
    if not await check_process_slot(update):
        return
    token = _secrets.token_hex(8)
...
async def run_recording_job(update, context, channel, duration_seconds,
    ...
    try:
        ...
        async def _upload_partial(out_path, rec_elapsed):
            ...
            try:
                upload_bot = _upload_bot(context, fsize, "Partial recording")
        ...
        try:
            upload_bot = _upload_bot(
                context, os.path.getsize(out_file), "Recording"
            )
...
async def _default_audio_send_upload(context, status_message, output_path, info):
    ...
    with open(output_path, "rb") as raw:
        upload_bot = _upload_bot(
            context, os.path.getsize(output_path), "Audio-track output"
        )
        wrapped = _ProgressUploadFile(raw, info)
        telegram_input = InputFile(
            wrapped, filename=os.path.basename(output_path), read_file_handle=False
...
async def _media_upload_output(context, chat_id: int, output_path: str,
                               caption: str = ""):
    upload_bot = _upload_bot(
        context, os.path.getsize(output_path), "Processed file"
    )
...
async def _media_local_job(update, context, source: dict, operation: str,
    ...
    size_error = _telegram_file_limit_error(source.get("file_size"), "The video")
    if size_error:
        await target_message.reply_text(size_error)
        return
...
def main():
    ...
    # Keep the application client on Telegram's normal API. Large-file
    # transfers are routed to the Local Bot API by _transfer_bot().
    _start_local_telegram_api()
    app_builder = Application.builder().token(BOT_TOKEN)
    logger.info(
        "Telegram transfer routing: files up to 20 MB use the normal Bot API; "
        "larger files use Local Bot API when enabled."
    )
