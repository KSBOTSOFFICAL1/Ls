...
blackbars_sessions: dict = {}  # {user_id: /BlackBars UI state}
blackbars_tasks: dict = {}     # {task_id: independent /BlackBars processing job}
...
# ---------------------------------------------------------------------------
# /BlackBars — process a replied video with quality/aspect + selectable timed watermark
# ---------------------------------------------------------------------------
...
# User-facing labels are kept exactly as requested.
_BLACKBARS_ASPECTS = {
    "bars169": "16:9 BlackBars",
    "bars45":  "4.5 BlackBars",
    "crop169": "16:9",
}

_BLACKBARS_WM_WINDOWS = {
    "10_60": {
        "label": "00:00:10 → 00:01:00 ✅",
        "start": 10,
        "end": 60,
    },
    "1800_1860": {
        "label": "00:30:00 → 00:31:00 ✅",
        "start": 1800,
        "end": 1860,
    },
}
...
_BLACKBARS_WM_POS = {
    # Watermark image: https://iili.io/CuMJCjn.md.png
    # Exact FFmpeg positions supplied for /BlackBars.
    "top_left": {
        "label": "Top-Left",
        "scale": 240,
        "xy": "(main_w-overlay_w)/2-6:120",
    },
    "left_corner": {
        "label": "Left Corner",
        "scale": 240,
        "xy": "290:main_h-overlay_h-120",
    },
}


_BLACKBARS_WATERMARK_URL = "https://iili.io/CuMJCjn.md.png"
_BLACKBARS_WATERMARK_CACHE = join(DATA_DIRECTORY, "blackbars_watermark.png")


async def _blackbars_ensure_watermark_img() -> bool:
    """Fetch the fixed /BlackBars watermark without changing global watermark settings."""
    if os.path.exists(_BLACKBARS_WATERMARK_CACHE) and os.path.getsize(_BLACKBARS_WATERMARK_CACHE) > 0:
        return True
    rc, _, err = await runcmd(
        f'curl -L -s --max-time 20 -o {shlex.quote(_BLACKBARS_WATERMARK_CACHE)} '
        f'{shlex.quote(_BLACKBARS_WATERMARK_URL)}'
    )
    ok = (rc == 0 and os.path.exists(_BLACKBARS_WATERMARK_CACHE)
          and os.path.getsize(_BLACKBARS_WATERMARK_CACHE) > 0)
    if not ok:
        LOG.warning("/BlackBars watermark download failed: %s", err[-300:])
    return ok


def _blackbars_default_state(user_id: int, src_msg, src_media) -> dict:
    return {
        "user_id": user_id,
        "src_msg": src_msg,
        "src_media": src_media,
...
def _blackbars_keyboard(state: dict) -> InlineKeyboardMarkup:
    uid = state["user_id"]
    q = state["quality"]
    asp = state["aspect"]
    ...
    def qbtn(key, label):
        return InlineKeyboardButton(
            f"✅ {label}" if q == key else label,
            callback_data=f"bb:{uid}:q:{key}",
        )

    def abtn(key):
        label = _BLACKBARS_ASPECTS[key]
        return InlineKeyboardButton(
            f"✅ {label}" if asp == key else label,
            callback_data=f"bb:{uid}:a:{key}",
        )

    def wbtn(key):
        label = _BLACKBARS_WM_WINDOWS[key]["label"]
        return InlineKeyboardButton(
            f"{label}" if window == key else label.replace(" ✅", ""),
            callback_data=f"bb:{uid}:w:{key}",
        )

    def pbtn(key):
        label = _BLACKBARS_WM_POS[key]["label"]
        return InlineKeyboardButton(
            f"✅ {label}" if pos == key else label,
            callback_data=f"bb:{uid}:p:{key}",
        )

    return InlineKeyboardMarkup([
        [qbtn("480", "480p"), qbtn("576", "576p")],
        [qbtn("720", "720p"), qbtn("1080", "1080p")],
        [abtn("bars169"), abtn("bars45")],
        [abtn("crop169")],
        [InlineKeyboardButton("💧 Watermark Interval", callback_data=f"bb:{uid}:noop")],
        [wbtn("10_60")],
        [wbtn("1800_1860")],
        [InlineKeyboardButton("📍 Watermark Position", callback_data=f"bb:{uid}:noop")],
        [pbtn("top_left"), pbtn("left_corner")],
        [
            InlineKeyboardButton("🆕 New", callback_data=f"bb:{uid}:new"),
            InlineKeyboardButton("▶️ Start", callback_data=f"bb:{uid}:start"),
        ],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"bb:{uid}:cancel")],
    ])


def _blackbars_text(state: dict) -> str:
    q = state["quality"]
    aspect = _BLACKBARS_ASPECTS[state["aspect"]]
    wm = _BLACKBARS_WM_POS[state["wm_pos"]]["label"]
    window = _BLACKBARS_WM_WINDOWS[state["wm_window"]]
    return (
        "**⬛ /BlackBars**\n\n"
        f"Quality: `{q}p`\n"
        f"Aspect: `{aspect}`\n\n"
        "**Watermark Interval**\n"
        f"Selected: `{window['label']}`\n\n"
        "**Watermark Position**\n"
        f"Selected: `{wm}`\n"
        "Watermark URL: `https://iili.io/CuMJCjn.md.png`\n\n"
        "Choose the options above, then press **Start**."
    )


def _blackbars_filter(state: dict) -> str:
    q = state["quality"]
    aspect = state["aspect"]
    out_w, out_h = _BLACKBARS_QUALITY[q]

    if aspect == "bars169":
        # Fit source into a 16:9 canvas and preserve the complete frame.
        vf = (
        ...
        # Existing 4:5 BlackBars logic is retained as a separate mode.
        canvas_w = max(2, round(out_h * 4 / 5))
        vf = (
            f"scale={canvas_w}:{out_h}:force_original_aspect_ratio=decrease,"
            f"pad={canvas_w}:{out_h}:(ow-iw)/2:(oh-ih)/2:black"
        )
    else:
        # Existing 16:9 mode: crop to 16:9, then scale to the selected quality.
        vf = (
...
def _blackbars_hms_seconds(value: str) -> int:
    return time_to_seconds(value)


def _blackbars_watermark_window(state: dict) -> tuple[int, int]:
    window = _BLACKBARS_WM_WINDOWS[state["wm_window"]]
    return int(window["start"]), int(window["end"])


def _blackbars_watermark_label(state: dict) -> str:
    start, end = _blackbars_watermark_window(state)
    return f"{_seconds_to_hms(start)} → {_seconds_to_hms(end)}"


def _blackbars_task_id(uid: int) -> str:
    # Unique per process; unlike user-id/time combinations this cannot collide
    # when multiple users start jobs in the same second.
    return f"bb_{uid}_{int(time.time() * 1000)}_{pysecrets.token_hex(4)}"


def _blackbars_progress_bar(pct: float, length: int = 10) -> str:
    pct = max(0.0, min(100.0, pct))
    filled = int(round((pct / 100.0) * length))
    return "█" * filled + "░" * (length - filled)


def _blackbars_speed_text(speed: float) -> str:
    return f"{speed:.2f}x" if speed and speed > 0 else "Calculating..."


def _blackbars_status_text(job: dict) -> str:
    duration = float(job.get("duration") or 0)
    current = float(job.get("current") or 0)
    pct = min(100.0, (current / duration) * 100.0) if duration > 0 else 0.0
    elapsed = max(0.0, time.time() - job.get("started_at", time.time()))
    speed = float(job.get("speed") or 0)
    remaining = max(0.0, (duration - current) / speed) if speed > 0 else 0.0
    username = job.get("username") or "—"
    uid = job.get("uid")
    filename = job.get("filename") or "video"
    status = job.get("status", "Processing...")
    return (
        "🎬 **Processing Video...**\n\n"
        f"📄 File:\n`{filename}`\n\n"
        f"Progress:\n[{_blackbars_progress_bar(pct)}] {pct:.2f}%\n\n"
        f"⚡ Speed:\n{_blackbars_speed_text(speed)}\n\n"
        f"Status:\n{status}\n\n"
        f"⏱ Elapsed: `{_ff_sec_to_hms(elapsed)}`\n"
        f"⏳ Remaining: `{_ff_sec_to_hms(remaining) if speed > 0 else 'Calculating...'}`\n"
        f"👤 Username: `{username}`\n"
        f"🆔 User ID: `{uid}`"
    )


def _blackbars_keyboard_running(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
...
def _blackbars_ffmpeg_bitrate(duration: float, audio_count: int,
                              target_mb: float = 340.0) -> int:
    """Calculate H.264 bitrate from target size, duration and AAC audio budget."""
    if duration <= 0:
        return 800
    audio_kbps = 128 * max(1, audio_count)
    # MB -> bits, then subtract audio bits. Reserve a small container overhead.
    target_bits = target_mb * 1024 * 1024 * 8
    audio_bits = audio_kbps * 1000 * duration
    overhead_bits = target_bits * 0.015
    video_bits = max(300_000, target_bits - audio_bits - overhead_bits)
    return max(300, int(video_bits / duration / 1000))


async def _blackbars_run_ffmpeg(job: dict, ffmpeg_args: list[str]) -> None:
    """Run one BlackBars FFmpeg process and update the same Telegram status message."""
    task_id = job["task_id"]
    proc = await asyncio.create_subprocess_exec(
        *ffmpeg_args,
        stdout=asyncio.subprocess.PIPE,
    ...
    job["proc"] = proc
    user_ffmpeg_pids[job["uid"]] = proc.pid
    ...
    try:
        while proc.returncode is None:
            try:
                while True:
                    ...
                    if key == "out_time_us":
                        try:
                            job["current"] = int(value) / 1_000_000
                        except ValueError:
                            pass
                    elif key == "speed" and value not in ("N/A", ""):
                        try:
                            job["speed"] = float(value.rstrip("x"))
                        except ValueError:
                            pass
            ...
            if now - last_edit >= 5.0:
                try:
                    await job["status_msg"].edit_text(
                        _blackbars_status_text(job),
                        reply_markup=_blackbars_keyboard_running(task_id),
                    )
                except Exception:
                    pass
                last_edit = now
        ...
        job["returncode"] = proc.returncode or 0
        job["stderr"] = stderr
    finally:
        user_ffmpeg_pids.pop(job["uid"], None)


async def _blackbars_process_task(client: Client, task_id: str) -> None:
    job = blackbars_tasks.get(task_id)
    if not job:
        return
    ...
    try:
        src_msg = job["src_msg"]
        src_media = job["src_media"]
        q = job["quality"]
        aspect = job["aspect"]
        pos = job["wm_pos"]
        wm_start, wm_end = job["wm_start"], job["wm_end"]

        job["status"] = "Downloading source..."
        await status_msg.edit_text(_blackbars_status_text(job), reply_markup=_blackbars_keyboard_running(task_id))
        dl_path = await src_msg.download(file_name=join(save_dir, f"src_{src_msg.id}"))

        if job.get("cancelled"):
            return

        job["status"] = "Probing source..."
        await status_msg.edit_text(_blackbars_status_text(job), reply_markup=_blackbars_keyboard_running(task_id))
        info = await _ffprobe_video(dl_path)
        duration = float(info.get("duration") or 0)
        source_height = int(info.get("video_height") or 0)
        audio_count = len(info.get("audio_streams") or [])
        if duration <= 0:
            raise Exception("Could not determine source video duration.")
        ...
        # The watermark image is fetched once and then used as FFmpeg input.
        job["status"] = "Fetching watermark image..."
        await status_msg.edit_text(_blackbars_status_text(job), reply_markup=_blackbars_keyboard_running(task_id))
        if not await _blackbars_ensure_watermark_img():
            raise Exception("Watermark image could not be downloaded.")
        wm_img = _BLACKBARS_WATERMARK_CACHE
        if not wm_img:
            raise Exception("Watermark image cache is unavailable.")

        base_vf = _blackbars_filter(job)
        wm_cfg = _BLACKBARS_WM_POS[pos]
        wm_expr = f"between(t,{wm_start},{wm_end})"
        fc = (
            f"[0:v]{base_vf}[blackbars];"
            f"[1:v]scale={wm_cfg['scale']}:-1[watermark];"
            f"[blackbars][watermark]overlay={wm_cfg['xy']}:enable='{wm_expr}'[outv]"
        )

        src_name = os.path.splitext(
            os.path.basename(
                getattr(src_media, "file_name", "")
                or getattr(src_media, "file_unique_id", "")
                or "video"
            )
        )[0] or "video"
        out_path = join(save_dir, f"{src_name}_BlackBars_{q}p.mp4")
        job["output_file"] = out_path
        job["filename"] = os.path.basename(out_path)

        # If 1080p is explicitly selected for a sub-1080 source, target the
        # requested approximate 300–380 MB range using a calculated bitrate.
        # Other qualities use CRF, so there is no blind upscaling unless the
        # user explicitly selected the higher output quality.
        video_args = ["-c:v", "libx264"]
        if q == "1080" and source_height and source_height < 1080:
            target_bitrate = _blackbars_ffmpeg_bitrate(duration, audio_count, 340.0)
            video_args += [
                "-b:v", f"{target_bitrate}k",
                "-maxrate", f"{int(target_bitrate * 1.15)}k",
                "-bufsize", f"{int(target_bitrate * 2)}k",
                "-preset", "veryfast",
            ]
        else:
            video_args += ["-crf", "23", "-preset", "veryfast"]
        ...
        await status_msg.edit_text(_blackbars_status_text(job), reply_markup=_blackbars_keyboard_running(task_id))
        await _blackbars_run_ffmpeg(job, ffmpeg_args)
        ...
        if job.get("returncode", 1) != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
            err = job.get("stderr", "").strip()
            raise Exception(f"FFmpeg failed (exit {job.get('returncode')}).\n{err[-1800:]}")
        ...
        await runcmd(
            f'ffmpeg -hide_banner -loglevel error -nostats -y '
            f'-ss {min(max(1, int(duration / 2)), max(1, int(duration) - 1))} '
            f'-i {shlex.quote(out_path)} -vframes 1 -q:v 2 {shlex.quote(thumb)}'
        )
        thumb_path = thumb if os.path.exists(thumb) else None

        aspect_label = _BLACKBARS_ASPECTS[aspect]
        wm_label = _blackbars_watermark_label(job)
        caption = (
            "⬛ **BlackBars Applied**\n\n"
            f"Quality: `{q}p`\n"
            f"Aspect: `{aspect_label}`\n"
            f"Watermark: `{wm_cfg['label']}`\n"
            f"Watermark Times: `{wm_label}`\n"
            f"Size: `{out_size_mb:.1f} MB`\n"
            f"Duration: `{_seconds_to_hms(int(duration))}`\n"
            "_Auto-deleted from server after 3 hours._"
        )

        # Reuse the bot's existing Telegram upload/progress infrastructure.
        await _run_upload_destination(
            client, uid, "tg", status_msg, out_path, caption, duration,
            thumb_path=thumb_path, save_dir=save_dir,
        )
        ...
        LOG.error(f"/BlackBars failed uid={uid} task={task_id}: {e}", exc_info=True)
        if job.get("cancelled"):
            try:
            ...
            if len(err_text) > 2500:
                err_text = "...[truncated]...\n" + err_text[-2500:]
            try:
                await status_msg.edit_text(f"❌ **/BlackBars failed.**\n\n`{err_text}`", reply_markup=None)
            except Exception:
                pass
    finally:
        user_tasks.pop(uid, None)
        user_ffmpeg_pids.pop(uid, None)
        progress_tasks.pop(uid, None)
        cancelled_users.discard(uid)
        blackbars_tasks.pop(task_id, None)
        if save_dir and os.path.exists(save_dir):
            if success:
                schedule_retention_cleanup(save_dir)
            else:
...
@app.on_message(filters.command(["BlackBars", "blackbars"]) & AUTH)
async def blackbars_cmd(client: Client, message: Message):
    user_id = message.from_user.id

    if not is_verified(user_id):
        return await message.reply_text(
            "You must be **verified** to use /BlackBars. Run /verify."
        )

    if user_id in user_tasks:
        return await message.reply_text(
            "You already have an active job. Use /statusme or /cancelme."
        )

    src_msg, src_media = _get_replied_video(message)
    if not src_media:
        return await message.reply_text(
            "**Reply to a video** with `/BlackBars`.\n\n"
            "Choose 480p / 576p / 720p / 1080p, "
            "16:9 BlackBars / 4.5 BlackBars / 16:9, "
            "a watermark interval, and a watermark position."
        )

    state = _blackbars_default_state(user_id, src_msg, src_media)
    blackbars_sessions[user_id] = state
    await message.reply_text(
        _blackbars_text(state),
        reply_markup=_blackbars_keyboard(state),
    )


@app.on_callback_query(filters.regex(
    r"^bb:(\d+):(q|a|w|p|new|start|cancel|noop)(?::([a-z0-9_]+))?$"
))
async def blackbars_callback(client: Client, cq: CallbackQuery):
    uid = int(cq.matches[0].group(1))
    action = cq.matches[0].group(2)
    value = cq.matches[0].group(3)

    if cq.from_user.id != uid:
        return await cq.answer("Yeh aapka session nahi hai.", show_alert=True)

    state = blackbars_sessions.get(uid)
    if not state:
        return await cq.answer(
            "Session expire ho gaya. Dobara /BlackBars karein.",
            show_alert=True,
        )

    if action == "noop":
        return await cq.answer("Watermark interval aur position neeche select karein.")

    if action == "cancel":
        blackbars_sessions.pop(uid, None)
        await cq.answer("Cancelled.")
        try:
            await cq.message.edit_text("❌ /BlackBars cancelled.")
        except Exception:
            pass
        return

    if action == "new":
        blackbars_sessions[uid] = _blackbars_default_state(
            uid, state["src_msg"], state["src_media"]
        )
        state = blackbars_sessions[uid]
        await cq.answer("New settings created.")
        try:
            await cq.message.edit_text(
                _blackbars_text(state),
                reply_markup=_blackbars_keyboard(state),
    ...
    if action == "q" and value in _BLACKBARS_QUALITY:
        state["quality"] = value
        await cq.answer(f"{value}p selected.")
    elif action == "a" and value in _BLACKBARS_ASPECTS:
        state["aspect"] = value
        await cq.answer(_BLACKBARS_ASPECTS[value])
    elif action == "w" and value in _BLACKBARS_WM_WINDOWS:
        state["wm_window"] = value
        await cq.answer(_BLACKBARS_WM_WINDOWS[value]["label"])
    elif action == "p" and value in _BLACKBARS_WM_POS:
        state["wm_pos"] = value
        await cq.answer(_BLACKBARS_WM_POS[value]["label"])
    elif action == "start":
        pass
    else:
        return await cq.answer("Invalid option.", show_alert=True)
    ...
    if uid in user_tasks:
        return await cq.answer(
            "Aapka ek job already chal raha hai. /cancelme use karein.",
            show_alert=True,
        )

    blackbars_sessions.pop(uid, None)
    task_id = _blackbars_task_id(uid)
    src_msg = state["src_msg"]
    src_media = state["src_media"]
    wm_start, wm_end = _blackbars_watermark_window(state)
    filename = (
        getattr(src_media, "file_name", None)
        or getattr(src_media, "file_unique_id", None)
        or "video"
    )

    save_dir = join(DOWNLOAD_DIRECTORY, f"blackbars_{uid}_{task_id[-8:]}")
    os.makedirs(save_dir, exist_ok=True)
    status = cq.message
    user_tasks[uid] = time.time()

    job = {
        "task_id": task_id,
        "uid": uid,
        "chat_id": status.chat.id,
        "username": f"@{cq.from_user.username}" if cq.from_user.username else cq.from_user.first_name or "—",
        "status_msg": status,
        "src_msg": src_msg,
        "src_media": src_media,
        "save_dir": save_dir,
    ...
    # Run independently so another user's BlackBars task is never blocked.
    asyncio.create_task(_blackbars_process_task(client, task_id))


@app.on_callback_query(filters.regex(r"^progress:(.+)$"))
async def blackbars_progress_callback(client: Client, cq: CallbackQuery):
    task_id = cq.matches[0].group(1)
    job = blackbars_tasks.get(task_id)
    if not job:
        return await cq.answer("Task not found or already finished.", show_alert=True, cache_time=0)
    if cq.from_user.id != job["uid"]:
        return await cq.answer("Not your task.", show_alert=True, cache_time=0)
    ...
    username = job.get("username") or "—"
    popup = (
        f"File: {job.get('filename', 'video')}\n"
        f"Progress: {pct:.2f}%\n"
        f"Status: {job.get('status', 'Processing...')}\n"
        f"Elapsed: {_ff_sec_to_hms(elapsed)}\n"
        f"Remaining: {_ff_sec_to_hms(remaining) if speed > 0 else 'Calculating...'}\n"
        f"Speed: {_blackbars_speed_text(speed)}\n"
        f"Username: {username}\n"
        f"User ID: {job['uid']}"
    )
    await cq.answer(popup, show_alert=True, cache_time=0)


@app.on_callback_query(filters.regex(r"^cancel:(.+)$"))
async def blackbars_cancel_callback(client: Client, cq: CallbackQuery):
    task_id = cq.matches[0].group(1)
    job = blackbars_tasks.get(task_id)
    if not job:
        return await cq.answer("Task not found or already finished.", show_alert=True, cache_time=0)
    ...
    if proc:
        try:
            ...
            pass
    await cq.answer("❌ Cancelling…", show_alert=False, cache_time=0)
...
_BOT_COMMANDS = [
    ...
    BotCommand("BlackBars",      "BlackBars + quality + timed watermark (reply to video)"),
